...
export const moviesApi = {
  ...
  tmdbSearch: (q, page = 1) =>
    unwrap(apiClient.get('/movies/tmdb/search', { params: { q, page } })),
};
...
export const screensApi = {
  listByCinema: (cinemaId) =>
    unwrap(apiClient.get(`/screens/cinema/${cinemaId}`)),
  get: (id) => unwrap(apiClient.get(`/screens/${id}`)),
  create: (cinemaId, body) =>
    unwrap(apiClient.post(`/screens/cinema/${cinemaId}`, body)),
  update: (id, body) => unwrap(apiClient.patch(`/screens/${id}`, body)),
  regenerateSeats: (id) =>
    unwrap(apiClient.post(`/screens/${id}/regenerate-seats`)),
  remove: (id) => unwrap(apiClient.delete(`/screens/${id}`)),
  hardRemove: (id) =>
    unwrap(apiClient.delete(`/screens/${id}/permanent`)),
};

export const seatsApi = {
  listByScreen: (screenId) =>
    unwrap(apiClient.get(`/seats/screen/${screenId}`)),
...
export const paymentsApi = {
  createIntent: (bookingId) =>
    unwrap(apiClient.post('/payments/intent', { bookingId })),
  confirm: (paymentIntentId) =>
    unwrap(apiClient.post('/payments/confirm', { paymentIntentId })),
  status: (paymentIntentId) =>
    unwrap(apiClient.get(`/payments/status/${paymentIntentId}`)),
};
...
export const adminApi = {
  ...
  setUserActive: (id, active) =>
    unwrap(apiClient.patch(`/admin/users/${id}/active`, { active })),
  listBookings: (params) =>
    unwrap(apiClient.get('/admin/bookings', { params })),
  listShows: (params) =>
    unwrap(apiClient.get('/admin/shows', { params })),
  listPayments: (params) =>
    unwrap(apiClient.get('/admin/payments', { params })),
};