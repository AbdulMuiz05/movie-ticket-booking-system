...
export default function HomePage() {
  ...
  const { data, loading } = useApi(() => moviesApi.list({ status: 'NOW_SHOWING', limit: 8 }), []);
  const upcoming = useApi(() => moviesApi.list({ status: 'UPCOMING', limit: 4 }), []);
          ...
          <div className="flex-1">
            ...
            {hero ? (
              <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-ink-200">
                ...
                <span className="inline-flex items-center gap-1.5">
                  <Clock className="h-4 w-4" /> {formatDuration(hero.duration)}
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <Ticket className="h-4 w-4" /> {hero.language}
                </span>
                <span className="text-ink-400">·</span>
                <span>{(hero.genre || []).slice(0, 3).join(' · ')}</span>
              </div>
            ) : null}

            <div className="mt-8 flex flex-wrap gap-3">
              <button
                onClick={() => navigate(hero ? `/movies/${hero._id}` : '/movies')}
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="section-subtitle mt-1">Book your seat for the latest releases</p>
          </div>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            action={<Link to="/movies" className="btn-outline">Browse movies</Link>}
          />
        ) : (
      ...
      {upcomingMovies.length > 0 ? (
        <section className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="mb-6">
            ...
            <p className="section-subtitle mt-1">Get ready for these upcoming releases</p>
          </div>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          ...
          <div className="card overflow-hidden">
            <div className="grid gap-0 md:grid-cols-2">
              ...
              <div className="flex flex-col justify-center gap-4 p-8">
                ...
                <h3 className="font-display text-4xl tracking-wide text-white">{hero.title}</h3>
                <p className="text-sm text-ink-300 line-clamp-4">{hero.description}</p>
                {hero.trailer ? (
                  <a
                    href={hero.trailer}
                    target="_blank"
                  ...
                  <Link to={`/movies/${hero._id}`} className="btn-primary w-fit">
