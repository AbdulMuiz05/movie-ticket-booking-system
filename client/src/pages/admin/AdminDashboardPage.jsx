...
export default function AdminDashboardPage() {
  ...
  const bookings =
    recentBookings.data?.data?.bookings || [];
  const shows =
    upcomingShows.data?.data?.shows || [];
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {cards.map((card) => (
          ...
          >
            <div
              className={`mb-3 inline-flex rounded-lg p-2 ${card.tone}`}
            >
              ...
              {shows.map((show) => (
                ...
                >
                  ...
                  <p className="mt-0.5 text-xs text-ink-400">
                    {show.cinema?.name || 'Unknown cinema'}{' '}
                    · {show.screen?.name || 'Unknown screen'}{' '}
